...
class TurnFaucetEnv(TurnFaucetBaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_ids = ['5029']  # model_ids  # ['5023']  # 5072, 5023, 5053 model_ids[:1]
        self.model_id = None
    ...
    def _get_obs_extra(self) -> OrderedDict:
        ...
        if self._obs_mode in ["state", "state_dict", "image"]:
            ...
            cmass_pose = self.target_link.pose * self.target_link.cmass_local_pose
            obs["wrapped_target_link_pose"] = vectorize_pose_v2(self.tcp.pose.inv() * cmass_pose)
            obs["wrapped_goal_pose"] = np.array([self.target_angle - self.current_angle] * 6)
            obs["wrapped_distance_to_target_object"] = np.array([self._compute_distance()] * 3)
            obs["wrapped_distance_to_init"] = np.array([self.current_angle - self.init_angle] * 3)
            obs["wrapped_distance_to_goal"] = np.array([self.target_angle - self.current_angle] * 3)
        return obs
