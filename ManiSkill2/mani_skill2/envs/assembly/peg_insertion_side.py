...
class PegInsertionSideEnv(StationaryManipulationEnv):
    ...
    def _load_actors(self):
        ...
        center = 0.5 * (length - radius) * np.array([0, 0])
    ...
    def _initialize_actors(self):
        ...
        self.init_peg_pose = Pose(pos, quat)
        self.peg.set_pose(self.init_peg_pose)

        # xy = self._episode_rng.uniform([-0.05, 0.2], [0.05, 0.4])
        xy = np.array([0.075, 0.225])
    ...
    def _get_obs_extra(self) -> OrderedDict:
        ...
        if self._obs_mode in ["state", "state_dict", "image"]:
            ...
            obs["wrapped_distance_to_target_object"] = tgt_gripper_pose.p - gripper_pos
            obs["wrapped_distance_to_goal"] = self.goal_pose.p - self.peg_head_pose.p
            obs["wrapped_distance_to_init"] = self.peg.pose.p - self.init_peg_pose.p
        return obs
    ...
    def compute_dense_reward(self, info, **kwargs):
        ...
        if rotated_properly:
            # reaching reward
            gripper_to_peg_dist = np.linalg.norm(
                gripper_pos - tgt_gripper_pose.p
            )
            ...
            if is_grasped:
                ...
                peg_head_wrt_goal_yz_dist = np.linalg.norm(
            ...
            if is_grasped and pre_inserted:
                ...
                insertion_reward = 5 * (
                    1
                    - np.tanh(
                        5.0 * np.linalg.norm(peg_head_wrt_goal_inside_hole.p)
                    )
                )
