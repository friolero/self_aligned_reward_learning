...
def vectorize_pose_v2(pose):
    return np.hstack(
        [pose.p, R.from_quat(pose.q[[1, 2, 3, 0]]).as_euler("xyz")]
    )
...
class PlugChargerEnv(StationaryManipulationEnv):
    ...
    def _build_charger(self, peg_size, base_size, gap):
        ...
        builder.add_box_visual(
            Pose([peg_size[0], gap, 0]), peg_size, material=mat
        )
        builder.add_box_collision(Pose([peg_size[0], -gap, 0]), peg_size)
        builder.add_box_visual(
            Pose([peg_size[0], -gap, 0]), peg_size, material=mat
        ...
        builder.add_box_visual(
            Pose([-base_size[0], 0, 0]), base_size, material=mat
        )
    ...
    def _load_actors(self):
        ...
        self.charger = self._build_charger(
            self._peg_size, self._base_size, self._peg_gap
        )
    ...
    def _initialize_task(self):
        self.goal_pose = self.receptacle.pose.transform(
            Pose(q=euler2quat(0, 0, np.pi))
        )
    ...
    def _get_obs_extra(self) -> OrderedDict:
        ...
        if self._obs_mode in ["state", "state_dict", "image"]:
            obs.update(
                ...
                wrapped_charger_pose=vectorize_pose_v2(self.charger.pose),
                wrapped_goal_pose=vectorize_pose_v2(self.goal_pose),
            )

            # charger_head_offset = Pose([self._base_size[0]/2, 0, 0])
            # charger_head_pose = self.charger.pose.transform(charger_head_offset)
            # heading_dir = self.charger.pose.p - charger_head_pose.p
            # heading_dir = heading_dir / np.linalg.norm(heading_dir)
            # obs["heading_dir"] = heading_dir

            obj_to_goal_dist, obj_to_goal_angle = self._compute_distance()
            obs["wrapped_distance_to_target_object"] = self.charger.pose.p - self.tcp.pose.p
            obs["wrapped_distance_to_goal"] = self.goal_pose.p - self.charger.pose.p
            obs["wrapped_distance_to_init"] = self.charger.pose.p - self._init_pos
        return obs
    ...
    def _compute_distance(self):
        ...
        obj_to_goal_angle = min(
            obj_to_goal_angle, np.pi * 2 - obj_to_goal_angle
        )
    ...
    def compute_dense_reward(self, info, **kwargs):
        ...
        tcp_rot_wrt_charger = tcp_pose_wrt_charger.to_transformation_matrix()[
            :3, :3
        ]
        ...
        if rotated_properly:
            ...
            if is_grasped:
                ...
                charger_cmass_wrt_goal_rot = charger_cmass_wrt_goal.to_transformation_matrix()[
                    :3, :3
                ]
                charger_wrt_goal = self.goal_pose.inv() * self.charger.pose
                charger_wrt_goal_yz_dist = np.linalg.norm(
                    charger_wrt_goal.p[1:]
                )
                charger_wrt_goal_dist = np.linalg.norm(charger_wrt_goal.p)
                charger_wrt_goal_rot = charger_cmass_wrt_goal.to_transformation_matrix()[
                    :3, :3
                ]
                ...
                pre_insertion_reward = 3 * (
                    ...
                    - np.tanh(
                        1.0
                        * (
                            charger_cmass_wrt_goal_yz_dist
                            + charger_wrt_goal_yz_dist
                        )
                        + 9.0
                        * np.maximum(
                            charger_cmass_wrt_goal_yz_dist,
                ...
                pre_insertion_reward += 3 * (
                    1 - np.tanh(3 * charger_wrt_goal_dist)
                )
                ...
                if pre_inserted:
                    insertion_reward = 2 * (
                        1 - np.tanh(25.0 * charger_wrt_goal_dist)
                    )
                    insertion_reward += 5 * (
