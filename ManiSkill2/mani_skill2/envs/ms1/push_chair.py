...
class PushChairEnv(MS1BaseEnv):
    ...
    def _ignore_collision(self):
        ...
        if (
            self.seat_link == self.support_link
        ):  # sometimes they are the same one
            return
        ...
        if chair_tilt < 0.2 * np.pi:
            ...
            if dist_ee_to_chair < 0.1:
                ...
                if dist_chair_to_target <= 0.15:
                    ...
                    reward += (
                        max(-1, 1 - np.exp(x)) * 2 - dist_chair_to_target * 2
                    )
        else:
            stage_reward = -5
    ...
    def _get_obs_extra(self):
        ...
        obs["wrapped_chair_pose"] = vectorize_pose_v2(self.chair.pose)

        obs["wrapped_goal_pose"] = vectorize_pose_v2(
            sapien.Pose(
                [self.target_xy[0], self.target_xy[1], self.chair.pose.p[2]]
            )
        )

        ee_coords = np.array(self.agent.get_ee_coords())  # [M, 3]
        self.chair_pcd = self._get_chair_pcd()
        chair_pcd = self._get_chair_pcd()  # [N, 3]
        dist_ees_to_chair = sdist.cdist(ee_coords, chair_pcd)
        distance_to_chair = ee_coords - chair_pcd[dist_ees_to_chair.argmin(1)]
        obs["wrapped_distance_to_target_object"] = np.concatenate(
            [
                distance_to_chair[: distance_to_chair.shape[0] // 2].mean(0),
                distance_to_chair[distance_to_chair.shape[0] // 2 :].mean(0),
            ]
        )
        ...
        obs["wrapped_distance_to_goal"] = np.array(
            [disp_chair_to_target[0], disp_chair_to_target[1], 0]
        )
