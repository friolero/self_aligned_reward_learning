...
class OpenCabinetEnv(MS1BaseEnv):
    ...
    def _set_cabinet_handles(self, joint_type: str):
        ...
        for link, joint in zip(
            self.cabinet.get_links(), self.cabinet.get_joints()
        ):
    ...
    def _set_target_link(self):
        ...
        joint_pose = (
            self.target_joint.get_global_pose().to_transformation_matrix()
        )
    ...
    def compute_dense_reward(self, *args, info: dict, **kwargs):
        ...
        # Rotation
        target_grasp_poses = self.target_handles_grasp_poses[
            self.target_link_idx
        ]
        ...
        if ee_close_to_handle:
            ...
            if not info["open_enough"]:
                ...
                reward_qvel = (
                    clip_and_normalize(link_qvel, -0.1, 0.5) * coeff_qvel
                )
                reward += reward_qvel
            else:
        ...
        # Update info
        info.update(
            ee_close_to_handle=ee_close_to_handle, stage_reward=stage_reward
        )
    ...
    def _get_obs_extra(self) -> OrderedDict:
        if self._init_link_pos is None:
            self._init_link_pos = self.target_link_pos
        ...
        cmass_pose = self.target_link.pose * self.target_link.cmass_local_pose
        obs["wrapped_target_link_pose"] = vectorize_pose_v2(
            self.tcp.pose.inv() * cmass_pose
        )
        obs["wrapped_goal_pose"] = np.array(
            [self.target_qpos - self.link_qpos] * 6
        )
        handle_pose = self.target_link.pose
        # [2, 10, 3]
        ee_coords = self.agent.get_ee_coords_sample().reshape(-1, 3)
        handle_pcd = transform_points(
            handle_pose.to_transformation_matrix(), self.target_handle_pcd
        )
        dist_ee_to_handle = sdist.cdist(ee_coords, handle_pcd)
        distance_to_handle = ee_coords - handle_pcd[dist_ee_to_handle.argmin(1)]
        obs["wrapped_distance_to_target_object"] = (
            distance_to_handle[: distance_to_handle.shape[0] // 2].mean(0)
            + distance_to_handle[distance_to_handle.shape[0] // 2 :].mean(0)
        ) / 2
        # obs["wrapped_distance_to_target_object"] = (
        #     self.tcp.pose.p - handle_pose.p
        # )
        obs["wrapped_distance_to_goal"] = np.array(
            [self.target_qpos - self.link_qpos] * 3
        )
        obs["wrapped_distance_to_init"] = np.array(
            [self.target_link_pos - self._init_link_pos] * 3
        )
...
class OpenCabinetDrawerEnv(OpenCabinetEnv):
    DEFAULT_MODEL_JSON = "{PACKAGE_ASSET_DIR}/partnet_mobility/meta/info_cabinet_drawer_train.json"
