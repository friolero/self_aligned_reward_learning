...
class MoveBucketEnv(MS1BaseEnv):
    ...
    def _load_agent(self):
        ...
        self.left_tcp: sapien.Link = get_entity_by_name(
            links, "left_panda_hand_tcp"
        )
        self.right_tcp: sapien.Link = get_entity_by_name(
    ...
    def _initialize_task(self):
        ...
        self.init_bucket_pose = self.bucket.pose
    ...
    def _initialize_bucket(self):
        ...
        self.bucket_body_link = self.bucket.get_active_joints()[
            0
        ].get_parent_link()
    ...
    def compute_dense_reward(self, action, info: dict, **kwargs):
        ...
        ees_height_diff = abs(
            (
                quat2mat(self.root_link.get_pose().q).T
                @ (ee_mids[0] - ee_mids[1])
            )[2]
        )
    ...
    def _get_obs_extra(self):
        ...
        if self._obs_mode not in ["state", "state_dict"]:
            obs["left_tcp_pose"] = vectorize_pose_v2(self.left_tcp.pose)
            obs["right_tcp_pose"] = vectorize_pose_v2(self.right_tcp.pose)
        obs["wrapped_bucket_pose"] = vectorize_pose_v2(self.bucket.pose)

        obs["wrapped_goal_pose"] = vectorize_pose_v2(
            sapien.Pose(
                [self.target_xy[0], self.target_xy[1], self.bucket.pose.p[2]]
            )
        )
        ...
        dist_ees_to_bucket = sdist.cdist(ee_coords, bucket_pcd)  # [M, N]
        distance_to_bucket = (
            ee_coords - bucket_pcd[dist_ees_to_bucket.argmin(1)]
        )
        obs["wrapped_distance_to_target_object"] = np.concatenate(
            [
                distance_to_bucket[: distance_to_bucket.shape[0] // 2].mean(0),
                distance_to_bucket[distance_to_bucket.shape[0] // 2 :].mean(0),
            ]
        )
        ...
        obs["wrapped_distance_to_goal"] = np.array(
            [disp_bucket_to_target[0], disp_bucket_to_target[1], 0]
        )
        obs["wrapped_distance_to_init"] = vectorize_pose_v2(
            self.bucket.pose
        ) - vectorize_pose_v2(self.init_bucket_pose)
        for idx in range(3, 6):
            if obs["wrapped_distance_to_init"][idx] < np.pi:
                obs["wrapped_distance_to_init"][idx] += 2 * np.pi
            if obs["wrapped_distance_to_init"][idx] > np.pi:
                obs["wrapped_distance_to_init"][idx] -= 2 * np.pi
        return obs
