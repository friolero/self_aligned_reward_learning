...
class StackCubeEnv(StationaryManipulationEnv):
    ...
    def _initialize_actors(self):
        ...
        cubeA_quat = euler2quat(0, 0, np.pi)
        cubeB_quat = euler2quat(0, 0, np.pi)
    ...
    def _get_obs_extra(self):
        obs = OrderedDict(
            tcp_pose=vectorize_pose_v2(self.tcp.pose),
        )
        if self._obs_mode in ["state", "state_dict", "image"]:
            ...
            obs.update(
                cubeA_pose=vectorize_pose(self.tcp.pose.inv() * self.cubeA.pose),
                cubeB_pose=vectorize_pose(self.tcp.pose.inv() * self.cubeB.pose),
                tcp_to_cubeA_pos=self.cubeA.pose.p - self.tcp.pose.p,
                tcp_to_cubeB_pos=self.cubeB.pose.p - self.tcp.pose.p,
                cubeA_to_cubeB_pos=self.cubeB.pose.p - self.cubeA.pose.p,
                wrapped_cubeA_pose=vectorize_pose_v2(self.tcp.pose.inv() * self.cubeA.pose),
                #wrapped_cubeB_pose=vectorize_pose_v2(self.tcp.pose.inv() * self.cubeB.pose),
                wrapped_goal_pose=vectorize_pose_v2(self.tcp.pose.inv() * sapien.Pose(goal_pos)),
            )
            obs["wrapped_distance_to_target_object"] = self.tcp.pose.p - self.cubeA.pose.p
            obs["wrapped_distance_to_goal"] = goal_pos - self.cubeA.pose.p
