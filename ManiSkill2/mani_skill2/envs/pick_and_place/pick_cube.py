    ...
    def _get_obs_extra(self) -> OrderedDict:
        ...
        if self._obs_mode in ["state", "state_dict", "image"]:
            obs.update(
                ...
                wrapped_obj_pose=vectorize_pose_v2(self.tcp.pose.inv() * self.obj.pose),
                wrapped_goal_pose=vectorize_pose_v2(self.tcp.pose.inv() * Pose(self.goal_pos)),
            )
            obs["wrapped_distance_to_target_object"] = self.tcp.pose.p - self.obj.pose.p
            obs["wrapped_distance_to_goal"] = self.goal_pos - self.obj.pose.p
