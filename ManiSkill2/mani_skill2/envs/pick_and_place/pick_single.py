...
class PickSingleEnv(StationaryManipulationEnv):
    ...
    def _get_obs_extra(self) -> OrderedDict:
        ...
        if self._obs_mode in ["state", "state_dict", "image"]:
            obs.update(
                ...
                obj_pose=vectorize_pose(self.tcp.pose.inv() * self.obj.pose),
                tcp_to_obj_pos=self.obj_pose.p - self.tcp.pose.p,
                obj_to_goal_pos=self.goal_pos - self.obj_pose.p,
                # obj_pose=vectorize_pose_v2(self.obj_pose),
                # goal_pose=vectorize_pose_v2(Pose(self.goal_pos)),
                wrapped_obj_pose=vectorize_pose_v2(self.obj_pose),
                wrapped_goal_pose=vectorize_pose_v2(
                    self.tcp.pose.inv() * Pose(self.goal_pos)
                ),
            )
            obs["wrapped_distance_to_target_object"] = self.tcp.pose.p - self.obj_pose.p
            obs["wrapped_distance_to_goal"] = self.goal_pos - self.obj_pose.p
        return obs
    ...
    def compute_dense_reward_legacy(self, info, **kwargs):
        ...
        if info["success"]:
            ...
            tcp_rot_wrt_obj = tcp_pose_wrt_obj.to_transformation_matrix()[
                :3, :3
            ]
            gt_rot_x_diff_1 = (
            ...
            bbox_x_loss = np.minimum(
                grasp_rot_loss_fxn(gt_rot_x_diff_1),
                grasp_rot_loss_fxn(gt_rot_x_diff_2),
            )
            bbox_y_loss = np.minimum(
                grasp_rot_loss_fxn(gt_rot_y_diff_1),
                grasp_rot_loss_fxn(gt_rot_y_diff_2),
            )
            grasp_rot_loss = 1.0
            should_rotate = False
            ...
            if rotated_properly:
                ...
                if grasp_from_above_height_offset > 0.01:
                    ...
                    tcp_to_obj_dist[2] += min(
                        self.model_bbox_size[2] / 2, 0.005
                    )
...
def build_actor_egad(
    ...
    collision_file = (
        Path(root_dir) / f"egad_{split}_set_coacd" / f"{model_id}.obj"
    )
    builder.add_multiple_collisions_from_file(
