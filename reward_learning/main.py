...
from mani_skill2_wrapper import flatten_state_dict
...
parser.add_argument("--learnt_reward_fn", type=str, default=None)
...
class RewardLearner:
    def __init__(self, cfg):
        ...
        print(f"start_step = {self.start_step}; n_iter = {self.n_iter}")
    ...
    def learn(
        ...
        **kwargs,
        ...
        while self.start_step < self.cfg.agent_params.total_steps:
            ...
            # sample from replay & rollout
            rollout_samples = self.sample_from_rollout(
                model,
                eval_env,
            ...
            # 3) learning from llm pairwise preference comparison
            ...
            # 3b) Otherwise collect sample from replay and learn via self-align
                ...
                if self.cfg.replay_feedback:
                    replay_samples = self.sample_from_replay(
                        model,
                        n_recent=model.replay_buffer.buffer_size,
                        sort=True,
                        start_high=True,
                    )
                    (
                    ...
                    # feedback & update reward function
                    self.pref_data.append(replay_pref)
                    self.pref_data.append(rollout_pref)
                else:
                    self.pref_data.append(rollout_pref)
                    self.pref_data.append(rollout_pref)
                self.hps = self_align(
...
if __name__ == "__main__":
    ...
    eval_env = SubprocVecEnv(
        [make_env(cfg) for i in range(cfg.agent_params.eval_num)]
    )
    ...
    if args.learnt_reward_fn is not None:
        with open(args.learnt_reward_fn, "rb") as fp:
            hp_history = pkl.load(fp)
        print("Hp history:", hp_history)
    else:
        hp_history = []
