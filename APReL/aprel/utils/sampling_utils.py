...
def uniform_weight_logprior(params: Dict) -> float:
    """
...
def linear_gaussian_proposal(point: Dict) -> Dict:
    """
    For the Metropolis-Hastings sampling algorithm, this function generates the
    next step in the Markov chain, with a Gaussian distribution of standard
    deviation 0.05.
...
def gaussian_proposal(point: Dict) -> Dict:
    ...
    for key, value in point.items():
        ...
        if getattr(value, "shape", None) is not None:
            ...
            shape = [1]
        next_point[key] = np.clip(value + np.random.randn(*shape) * 0.2, 0, 1)
        #    next_point[key] /= np.linalg.norm(next_point[key])
    return next_point
