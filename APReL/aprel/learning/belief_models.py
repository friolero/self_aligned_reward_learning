...
class SamplingBasedBelief(LinearRewardBelief):
    ...
    def update(
        self,
        data: Union[QueryWithResponse, List[QueryWithResponse]],
        initial_point: Dict = None,
    ):
        """
    ...
    def create_samples(
        ...
        curr_logprob = self.logprior(
            curr_point
        ) + sampling_user.loglikelihood_dataset(self.dataset)
        ...
        for _ in range(burnin + thin * self.num_samples - 1):
            ...
            samples.append(curr_point)
            logprobs.append(curr_logprob)
        self.samples, self.logprobs = (
            samples[burnin::thin],
            logprobs[burnin::thin],
        )
