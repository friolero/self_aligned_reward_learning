...
class RewardLearner:
    def __init__(self, cfg):
        ...
        if cfg.pretrained_fn != "":
            self.start_step = int(
                cfg.pretrained_fn.split("/")[-1].split(".")[0].replace("model_sac_", "")
            )
        else:
    ...
    def learn(
        ...
        **kwargs,
        ...
        while self.start_step < self.cfg.agent_params.total_steps:
            ...
            if (
                sum(rollout_successes) / float(len(rollout_successes))
                >= 0.5
            ):
                print(
...
if __name__ == "__main__":
    ...
    if args.wandb_run_id is not None:
        if not os.path.isfile(cfg.pretrained_fn):
            pretrained_files = glob.glob(f"ckpts/{cfg.wandb_group}/{cfg.wandb_run_name}/model_sac_*.zip")
