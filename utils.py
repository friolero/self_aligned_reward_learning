...
from tulip.envs import (ManiSkill2DualArmMobileBaseTaskWrapper,
                        ManiSkill2FixedBaseTaskWrapper,
                        ManiSkill2MobileBaseTaskWrapper)
from tulip.policies import LinearScheduler, Logger, ReplayBuffer
from tulip.policies.ddpg import DrQV2
from tulip.policies.sac import SoftActorCritic
from tulip.policies.td3 import TwinDelayedDDPG
...
def init_agent(env, algo, cfg, pretrained_fn):
    stddev_schedule = LinearScheduler(
        cfg.stddev_min, cfg.stddev_max, cfg.stddev_steps
    )
    assert algo in ["drq_v2", "td3", "sac"], "RL algo {algo} is not supported."
    if algo == "drq_v2":
        algo_class = DrQV2
    elif algo == "td3":
        algo_class = TwinDelayedDDPG
    elif algo == "sac":
    algo_class = SoftActorCritic
    agent = algo_class(
        observation_space=env.observation_space,
        action_space=env.action_space,
        image_feat_dim=cfg.image_feat_dim,
        numerical_feat_dim=cfg.numerical_feat_dim,
...
def init_replay(cfg, env, agent):
    replay_buffer = ReplayBuffer(
        observation_space=env.observation_space,
        action_space=env.action_space,
        max_capacity=cfg.agent_params.buffer_size,
        in_memory=cfg.agent_params.in_memory_buffer,
        root_dir=f"data/{cfg.wandb_group}/{cfg.wandb_run_name}",
        rgb_transforms=agent.rgb_transforms,
        depth_transforms=agent.depth_transforms,
    )
...
def sample_from_replay_hist(
    ...
    next_obses = retrieve_in_memory_next_obs(
        replay_source, sample_pos_indices
    )
    ...
    for idx in sample_pos_indices:
        step_var = {
            k: v[idx] for k, v in replay_source._reward_vars.items()
        }
